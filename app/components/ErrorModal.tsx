...
export function ErrorModal({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 dark:bg-black dark:bg-opacity-60 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center relative" style={{ display: 'inline-block', maxWidth: '90vw', minWidth: `${modalMinWidth}px` }}>
        <button
          onClick={onClose}
        ...
        <h3 className="text-lg font-semibold mb-6 whitespace-nowrap dark:text-white">{title}</h3>
        <div className="mb-6 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md" style={{ wordWrap: 'break-word', overflowWrap: 'break-word' }}>
        ...
        <div ref={buttonsContainerRef} className={`flex justify-center items-center ${secondaryAction ? 'gap-4' : ''}`} style={{ width: 'max-content', minWidth: '400px', margin: '0 auto' }}>
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-md bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition text-gray-700 dark:text-gray-300 whitespace-nowrap flex-shrink-0"
            disabled={secondaryAction?.processing}
          >
            Close
          </button>
          {secondaryAction && (
            <button
              ...
              className="px-4 py-2 rounded-md bg-red-600 dark:bg-red-700 hover:bg-red-700 dark:hover:bg-red-600 transition text-white flex items-center justify-center whitespace-nowrap"
              disabled={secondaryAction.processing}
            >
