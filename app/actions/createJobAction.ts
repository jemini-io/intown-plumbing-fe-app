...
import {
  Jpm_V2_CustomFieldModel,
  Jpm_V2_UpdateJobRequest,
} from "@/lib/servicetitan/generated/jpm";
...
export async function createJob(
  ...
  try {
    if (originalTechnician && !originalTechnician.isManagedTech) {
      const managedTechnician = await getTechnician(
        config.defaultManagedTechId
      );
