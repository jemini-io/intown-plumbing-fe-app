    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium dark:text-white">+{selectedCountry.dialCode}</span>
        </div>
        <ChevronDownIcon
              ...
              filteredCountries.map((country) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors ${
                    selectedCountry.code === country.code ? "bg-blue-100 dark:bg-gray-700" : ""
                  }`}
                >
                  <span className="text-xl">{country.flag}</span>
                  <span className="text-sm font-medium dark:text-white">+{country.dialCode}</span>
                </button>
              ))
            ) : (
